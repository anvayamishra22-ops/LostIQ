...
const LostItems = () => {
  ...
  useEffect(() => {
    // Implement a simple debounce or fetch on changes
    fetchItems();
  ...
  const handleReset = () => {
    ...
    setLoading(true);
    itemAPI.getAll({ type: 'lost' }).then(response => {
      setItems(response.data);
      setLoading(false);
    }).catch(err => {
      setError('Failed to fetch lost items.');
  ...
  return (
    <div>
      <h2 className="section-title">Reported Lost Items</h2>
      <p className="subtitle">Browse items reported lost on college campus. Help fellow students find them!</p>
      ...
      <form className="filters-bar" onSubmit={handleSearchSubmit}>
        <div className="filter-item" style={{ flex: 2 }}>
          ...
          <div style={{ display: 'flex', gap: '0.5rem' }}>
            <input
              ...
              placeholder="e.g. iPhone, Notebook, Keyring"
            />
        ...
        <div className="filter-item">
          ...
          >
            ...
            <option value="active">Still Lost</option>
        ...
        <>
          {items.length === 0 ? (
            <div className="card" style={{ padding: '3rem', textAlign: 'center', color: 'var(--text-light)' }}>
              No lost items match your search or filter settings.
            </div>
          ) : (
            <div className="grid">
              {items.map(item => (
